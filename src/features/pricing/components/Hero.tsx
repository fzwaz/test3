...
const PRICING_SLIDES = [
  { name: "Two motions", oneLiner: "Sales-assisted for Pulse/Fortress/Compass and self-serve checkout for GRC & DMARC." },
  { name: "Razorpay · Stripe", oneLiner: "India via Razorpay, Saudi/Bahrain & global via Stripe — auto-selected by billing country." },
  { name: "Illustrative structure", oneLiner: "Base + per-framework + seats + Pulse add-on — placeholders, not final prices." },
  { name: "Portal", oneLiner: "Self-serve or enterprise — same shell, entitlements scoped to what you purchased." },
];
...
export default function PricingHero() {
  ...
  const total = PRICING_SLIDES.length;
  ...
  const slide = PRICING_SLIDES[activeIdx];
  const scrollToPlans = () => document.getElementById("pricing-plans")?.scrollIntoView({ behavior: "smooth", block: "start" });
  const scrollToPortal = () => document.getElementById("pricing-portal")?.scrollIntoView({ behavior: "smooth", block: "start" });
          ...
          <div className="lg:w-[58%] flex flex-col justify-center items-start">
            <div style={textStyle(700)} className="mb-7">
              <div className="inline-flex items-center gap-2.5 px-4 py-[7px] rounded-full bg-black/60 border border-orange-500/25 text-orange-400 text-xs font-semibold tracking-[0.22em] uppercase shadow-[0_0_24px_rgba(249,115,22,0.25)] backdrop-blur-md">
                ...
                <span>Pricing — Accounts, Subscriptions & Payments</span>
              </div>
            </div>
            ...
            <h1 className="text-[32px] sm:text-[44px] lg:text-[40px] xl:text-[48px] font-bold text-[#f4f1ed] tracking-[-0.02em] leading-[1.08] select-none">
              <span className="block overflow-hidden pb-1"><span className="block lg:whitespace-nowrap" style={textStyle(900)}>Simple, modular</span></span>
              <span className="block overflow-hidden pb-2"><span className="block lg:whitespace-nowrap" style={textStyle(1050)}>pricing — <span className="text-[#ff7d1c]">your way.</span></span></span>
              <span className="block overflow-hidden"><span className="block text-[22px] sm:text-[26px] lg:text-[24px] xl:text-[28px] font-bold tracking-[-0.015em] text-[#f4f1ed] lg:whitespace-nowrap" style={textStyle(1200)}>Self-serve or <span className="text-[#ff7d1c]">sales-assisted.</span></span></span>
            </h1>

            <p className="mt-6 max-w-[560px] text-[15px] lg:text-base text-[#cfc9c2] font-normal leading-[1.65]" style={textStyle(1300)}>
              Choose how you buy — provisioned after a demo for the full platform, or checkout-to-dashboard for GRC and
              DMARC. Razorpay for India, Stripe for everywhere else.
            </p>

            <div className="mt-9 flex flex-col sm:flex-row items-stretch sm:items-center gap-4" style={textStyle(1500)}>
              <button type="button" onClick={scrollToPlans} className="group inline-flex items-center justify-center gap-3 px-7 py-[15px] rounded-[10px] bg-black/70 text-white font-semibold text-[15px] border border-[#f97316]/70 shadow-[0_0_20px_rgba(249,115,22,0.15)] hover:border-orange-400 hover:shadow-[0_0_28px_rgba(249,115,22,0.35)] hover:bg-[#140c06] transition-all duration-200 active:scale-[0.98] backdrop-blur-xl cursor-pointer">
                <span>See plans</span><ArrowRight className="w-[18px] h-[18px] stroke-[2.5] text-orange-400 group-hover:translate-x-1 transition-transform" />
              </button>
              <button type="button" onClick={scrollToPortal} className="group inline-flex items-center justify-center gap-3 px-7 py-[15px] rounded-[10px] bg-black/70 text-white font-semibold text-[15px] border border-white/15 hover:border-orange-400 hover:shadow-[0_0_28px_rgba(249,115,22,0.2)] hover:bg-[#140c06] transition-all duration-200 active:scale-[0.98] backdrop-blur-xl cursor-pointer">
                <span>Portal & billing</span><ArrowRight className="w-[18px] h-[18px] stroke-[2.5] text-orange-400 group-hover:translate-x-1 transition-transform" />
              </button>
            </div>

            <p className="mt-4 text-xs font-mono text-slate-500" style={textStyle(1500)}>Illustrative structure — actual pricing TBD · See Section 9</p>
          </div>
